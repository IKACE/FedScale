...
class Client(object):
    ...
    def train(self, client_data, model, conf):
        ...
        results["utility"] = math.sqrt(self.loss_squre) * float(trained_unique_samples)
    ...
    def train_step(self, client_data, conf, model, optimizer, criterion):
        ...
        for data_pair in client_data:
            ...
            if conf.task == "detection":
                ...
                (data, masks) = data
                data, masks = Variable(data).to(device=conf.device), Variable(masks).to(
                    device=conf.device
                )
            ...
            self.loss_squre = sum([l**2 for l in loss_list]) / float(len(loss_list))
